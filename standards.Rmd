...
stds.meta %>% count(year, batch) %>% 
  kable(caption="chromatograms in each batch")
```
...
walk(mergedmix, function(chem) {
  meta.mix %>% filter(str_detect(type,"Standards")) %>% #also consider Indole, PineneOctanone types
  ggplot(aes(x=conc, group=datebatch, color=year, y=.data[[chem]])) +
